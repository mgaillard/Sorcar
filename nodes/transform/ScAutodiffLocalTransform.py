...
class ScAutodiffLocalTransform(Node, ScObjectOperatorNode):
    ...
    def functionality(self):
        ...
        #r = self.inputs["Radius"].default_value
        #s = self.inputs["Scale"].default_value
        #N = self.inputs["Number"].default_value
        #N = int(N)
        #print("eval r {} s {} N {}".format(r,s,N))
        ...
        for index, inst in enumerate(self.instances):
            ...
            #autodiff_variables.get_axis_system(box_name).translate(r, s, 0)

            #bpy.context.object.location[0] = np.sin(t) * r 
            #bpy.context.object.location[1] = np.cos(t) * r
            #bpy.context.object.scale = (s,s,s)
            inst.select_set(False)


        #for bb in self.id_data.get_object_autodiff_boxes_names():
        #    print(bb)
